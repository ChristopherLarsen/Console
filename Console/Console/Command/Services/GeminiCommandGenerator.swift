...
struct GeminiCommandGenerator: LLMCommandGenerator {
    ...
    func generateCommand(from naturalLanguage: String) async throws -> Command {
        GeminiCommandGenerator.lastRawResponse = nil

        let client = GeminiClient(apiKey: apiKey, config: config)
        let text = try await client.sendMessage(
            systemPrompt: CommandGeneratorPrompt.system,
            userMessage: naturalLanguage
        )

        GeminiCommandGenerator.lastRawResponse = text
