...
| **Home** | `Home/` | Two-panel grid: JIRA My Tickets · Agent Sessions (`HomePanel` enum in `HomeView.swift`). Sessions quadrant = attention radar/launcher over `SessionStore` (no terminal embedded, no destructive controls). Shared card grammar per `Design/HomeCards/DESIGN_PROMPT.md` §3. |
