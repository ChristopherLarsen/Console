...
| **Terminal drawer** | `Terminal/` | Global bottom `zsh` drawer on every destination, toggled by the sidebar's "Main Terminal" item pinned above Settings. A retracted drawer is fully unmounted (zero reserved space); the shell survives inside `TerminalSessionManager`. Owned by `TerminalSessionManager` — process ownership is NEVER shared with `SessionStore`. Two independent SwiftTerm views can be visible at once (drawer + selected session PTY). |
...
| **Ticket Work** | `TicketWorkflow/` | Seven-stage ticket handling (Understand→Close): pure reducer/domain, opaque HMAC ticket association, minimal durable progress (see security-boundaries §2a), Jira detail-status observation via visible WebView DOM only, adapters over `IOSWorkflow` jobs/results/simulator, Ticket Work sidebar UI, and Next step targets. No Jira/GitLab REST; no ticket/MR text in prompts, argv, logs, or durable records. |
| **Settings** | `Settings/` | SettingsView sections incl. AI provider config, model selection, hotkey recorder, sessions section, iOS project profile, compact iOS jobs panel, WebView URL fields (JIRA/GitLab). |
